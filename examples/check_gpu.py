...
if __name__ == "__main__":
    ...
    if gpu_info:
        ...
        if gpu_info.get('tokens_per_second'):
            print(f"  • Tokens per second: {gpu_info['tokens_per_second']}")
            print("\n" + "=" * 60)
            print("\nPerformance Notes:")
            print("  • GPU acceleration typically achieves >50 tokens/second")
            print("  • CPU-only inference is typically <20 tokens/second")
            print("  • Actual performance varies by model size and hardware")
            
            if gpu_info['tokens_per_second'] > 50:
                print("\n✓ Performance indicates GPU acceleration is likely active!")
            elif gpu_info['tokens_per_second'] > 20:
